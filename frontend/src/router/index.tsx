...
//   пользователь переходит на /dashboard
//     → react-router-dom сопоставляет URL с <Route path="/dashboard">
//       → рендерит <ProtectedRoute> → проверяет есть ли user в AppStore
//         → если нет → редирект на /login
//         → если есть → lazy-импорт Dashboard.tsx → Suspense показывает заглушку
//           → чанк загружен → рендерится Dashboard
//
// LAZY LOADING (React.lazy + dynamic import):
//   каждая страница — отдельный JS-файл (чанк)
//   при первом открытии страницы браузер скачивает только её код
//   это ускоряет начальную загрузку: не качаем AdminPanel если юзер студент
//
// УРОВНИ ДОСТУПА:
//   без ProtectedRoute — публичные (лендинг, логин, регистрация)
//   ProtectedRoute без allowedRoles — любой залогиненный
//   allowedRoles=["teacher","admin"] — только учителя и выше
