...
const CATEGORY_META: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
  Programming: { label: "Программирование", icon: <Code2 size={14} />, color: "text-blue-600 bg-blue-50 dark:text-blue-400 dark:bg-blue-900/20" },
  Design: { label: "Дизайн", icon: <Palette size={14} />, color: "text-pink-600 bg-pink-50 dark:text-pink-400 dark:bg-pink-900/20" },
  "Data Science": { label: "Data Science", icon: <FlaskConical size={14} />, color: "text-violet-600 bg-violet-50 dark:text-violet-400 dark:bg-violet-900/20" },
  DevOps: { label: "DevOps", icon: <Container size={14} />, color: "text-orange-600 bg-orange-50 dark:text-orange-400 dark:bg-orange-900/20" },
  QA: { label: "Тестирование", icon: <ShieldCheck size={14} />, color: "text-emerald-600 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-900/20" },
  Analytics: { label: "Аналитика", icon: <BarChart3 size={14} />, color: "text-cyan-600 bg-cyan-50 dark:text-cyan-400 dark:bg-cyan-900/20" },
};

const DEFAULT_CAT = { label: "Другое", icon: <GraduationCap size={14} />, color: "text-[var(--muted)] bg-[var(--surface)]" };
...
export default function Course() {
  ...
  const [discussionMessages, setDiscussionMessages] = useState<DiscussionMessage[]>([
    { id: 1, author: "Куратор", text: "Пишите вопросы по шагу: по теории, тесту или коду.", createdAt: new Date().toISOString() },
  ]);
    ...
    try {
      ...
      try {
        ...
        for (const course of data) {
          if (progressMap[course.id] !== undefined) course.progress = progressMap[course.id];
        }
      } catch { /* not logged in */ }
      setCourses(data);
    } catch (err) {
  ...
  const loadCourseContent = async (cId: number) => {
    ...
    setStepError("");
    try {
      const data = await api.get<CourseContentResponse>(`/courses/${cId}/steps`);
      setCourseContent(data);
      const firstStepId = data.steps[0]?.id ?? null;
      const selectedStillExists = data.steps.some((s) => s.id === selectedStepId);
      const requestStepExists = requestedStepId && data.steps.some((s) => s.id === requestedStepId);
      if (requestStepExists) setSelectedStepId(requestedStepId);
      else setSelectedStepId(selectedStillExists ? selectedStepId : firstStepId);
      if (!selectedStillExists) setStepAnswer("");
      setAttemptHistory((prev) => prev.filter((a) => data.steps.some((s) => s.id === a.stepId)));
    } catch (err) {
  ...
  const loadEnrollmentStatus = async (cId: number) => {
    try {
      const data = await api.get<EnrollmentStatus>(`/courses/${cId}/enrollment-status`);
  ...
  const requestEnrollment = async () => {
    ...
    try {
      const result = await api.post<{ success?: boolean; message?: string; error?: string }>(
        `/courses/${selectedCourseId}/request-enrollment`, { message: enrollRequestMessage }
      );
    ...
    } finally { setEnrollRequestLoading(false); }
  };

  const createCourse = async () => {
    if (!newCourseTitle.trim()) { setError("Введите название курса"); return; }
    try {
      await api.post("/courses", { title: newCourseTitle.trim(), type: newCourseType, level: newCourseLevel });
      await loadCourses();
  ...
  const enrollCourse = async (cId: number) => {
    try {
      const result = await api.post<{ success?: boolean; error?: string }>(`/courses/${cId}/enroll`, {});
      if (result.error) toast.error(result.error);
      else {
    ...
    } catch (err) { toast.error(err instanceof Error ? err.message : "Не удалось записаться"); }
  };
  ...
  const submitStep = async () => {
    ...
    const currentStepIndex = courseContent?.steps.findIndex((s) => s.id === selectedStepId) ?? -1;
    const nextStepCandidateId =
      currentStepIndex >= 0 && courseContent && currentStepIndex < courseContent.steps.length - 1
        ...
        : null;
    try {
      const response = await api.post<{ passed: boolean; feedback: string; checkResults?: Array<{ name: string; passed: boolean }> | null }>(
        `/steps/${selectedStepId}/check`, { answer: stepAnswer }
      );
      const failedChecks = (response.checkResults ?? []).filter((c) => !c.passed);
      ...
      setAttemptHistory((prev) => [
        { id: Date.now(), stepId: selectedStepId, answer: stepAnswer, passed: response.passed, feedback: response.feedback, createdAt: new Date().toISOString() },
        ...prev,
      ]);
  ...
  const postDiscussionMessage = () => {
    ...
    if (!text) { toast.error("Введите текст сообщения"); return; }
    setDiscussionMessages((prev) => [{ id: Date.now(), author: "Вы", text, createdAt: new Date().toISOString() }, ...prev]);
    setDiscussionText("");
  };

  const activeStep = courseContent?.steps.find((s) => s.id === selectedStepId) ?? null;
  const activeProgress = courseContent?.progress.find((p) => p.stepId === selectedStepId) ?? null;
  const activeStepIndex = courseContent?.steps.findIndex((s) => s.id === selectedStepId) ?? -1;
  ...
  const nextStep = activeStepIndex >= 0 && courseContent && activeStepIndex < courseContent.steps.length - 1
    ? courseContent.steps[activeStepIndex + 1] : null;

  const submitLabel = activeStep
    ? activeStep.kind === "theory" ? "Отметить как изученный"
    : activeStep.kind === "quiz" ? "Проверить ответ"
    : "Отправить код"
    : "Проверить";

  const syllabusModules = useMemo(() => {
    if (!courseContent) return [] as Array<{ id: number; title: string; lessons: Array<{ id: number; title: string; steps: CourseStep[] }> }>;
    ...
    for (const step of courseContent.steps) {
      ...
      if (!lessons.has(lessonIndex)) lessons.set(lessonIndex, { id: lessonIndex, title: `Урок ${lessonIndex}`, steps: [] });
      lessons.get(lessonIndex)?.steps.push(step);
    }
    const modulesMap = new Map<number, { id: number; title: string; lessons: Array<{ id: number; title: string; steps: CourseStep[] }> }>();
    for (const lesson of Array.from(lessons.values())) {
      const moduleIndex = Math.floor((lesson.id - 1) / 2) + 1;
      if (!modulesMap.has(moduleIndex)) modulesMap.set(moduleIndex, { id: moduleIndex, title: `Модуль ${moduleIndex}`, lessons: [] });
  ...
  useEffect(() => {
    if (syllabusModules.length === 0) { setOpenModules({}); return; }
    setOpenModules((prev) => {
      ...
      return (
        <MainLayout>
          <div className="card p-6 max-w-lg">
            <p className="text-sm text-red-500 font-medium mb-4">{contentError}</p>
            <button onClick={() => navigate("/course")} className="btn-secondary text-sm px-4 py-2">
              ← Назад к каталогу
            </button>
          </div>
        </MainLayout>
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5 text-xs text-[var(--muted)]">
                  <span>{completed}/{total} шагов</span>
            ...
            <div className="card p-5 border-primary/20 bg-primary-50 dark:bg-primary-900/10">
              {enrollmentStatus.requestStatus === "pending" ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm text-[var(--text)]">Заявка на рассмотрении</p>
                    <p className="text-xs text-[var(--muted)] mt-0.5">Преподаватель рассмотрит вашу заявку</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-3">
                  <p className="font-semibold text-sm text-[var(--text)]">Запишитесь на курс, чтобы начать обучение</p>
                  <div className="flex gap-2 flex-wrap">
                    <button onClick={() => enrollCourse(selectedCourseId!)} className="btn-primary px-4 py-2 text-sm">
                      Записаться
                    </button>
            ...
            <div className="space-y-4 min-w-0">
                ...
                <>
                  {!activeStep && !contentLoading && (
                    <div className="card p-10 flex flex-col items-center justify-center text-center">
                      ...
                      <p className="font-semibold text-[var(--text)] mb-1">Выберите шаг</p>
                      <p className="text-sm text-[var(--muted)]">Нажмите на шаг в содержании курса справа</p>
                    </div>
                  )}
                    ...
                    <div className="card p-6 space-y-5" id="active-step-container">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`badge ${
                              activeStep.kind === "theory" ? "badge-neutral" :
                              activeStep.kind === "quiz" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400" :
                              "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"
                            } text-[11px] px-2 py-0.5 rounded-md font-semibold`}>
                              {activeStep.kind === "theory" ? "Теория" : activeStep.kind === "quiz" ? "Тест" : "Практика"}
                            </span>
                            <span className="text-xs text-[var(--muted)]">Шаг {activeStep.stepOrder}</span>
                            {activeStep.xp > 0 && (
                              <span className="flex items-center gap-0.5 text-xs font-semibold text-amber-500">
                                <Zap size={11} />{activeStep.xp} XP
                              </span>
                            )}
                          </div>
                          <h2 className="font-display font-bold text-lg text-[var(--text)]">{activeStep.title}</h2>
                        </div>
                      ...
                      {activeStep.kind === "theory" && (
                        <div className="prose prose-sm dark:prose-invert max-w-none text-[var(--text)] leading-relaxed text-sm whitespace-pre-wrap">
                          {activeStep.theoryText || <span className="text-[var(--muted)] italic">Теоретический материал будет добавлен.</span>}
                        </div>
                      )}
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                                  stepAnswer === option ? "border-primary bg-primary" : "border-[var(--border)]"
                                }`}>
                                  {stepAnswer === option && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                                </div>
                                {option}
                              </div>
                            </button>
                        ...
                        <div className="space-y-3">
                          ...
                          {activeStep.checks && activeStep.checks.length > 0 && (
                            <div className="p-4 rounded-xl bg-[var(--surface)] border border-[var(--border)] space-y-1.5">
                              <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider mb-2">Требования</p>
                              {activeStep.checks.map((check, idx) => (
                                <div key={idx} className="flex items-center gap-2 text-sm text-[var(--text)]">
                                  <div className="w-4 h-4 rounded border border-[var(--border)] bg-[var(--bg)] flex items-center justify-center shrink-0">
                                    <span className="text-[9px] text-[var(--muted)]">{idx + 1}</span>
                                  </div>
                                  {check}
                                </div>
                              ))}
                      ...
                      <div className="flex items-center justify-between gap-3 pt-2 border-t border-[var(--border)]">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={submitStep}
                            disabled={stepLoading || (activeStep.kind !== "theory" && !stepAnswer.trim())}
                            className="btn-primary px-5 py-2.5 text-sm gap-2"
                          >
                            {stepLoading
                              ? <span className="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                              : <><span>{submitLabel}</span>{nextStep && <ArrowRight size={14} />}</>
                            }
                          </button>
                        </div>
                      </div>
                  ...
                  {/* Attempt history */}
                  {attemptHistory.filter((a) => a.stepId === selectedStepId).length > 0 && (
                    <div className="card p-5 space-y-3">
                      <p className="text-xs font-semibold text-[var(--muted)] uppercase tracking-wider">История попыток</p>
                      <div className="space-y-2 max-h-48 overflow-auto">
                        {attemptHistory.filter((a) => a.stepId === selectedStepId).map((attempt) => (
                          <div key={attempt.id} className={`flex items-start gap-3 px-3 py-2.5 rounded-xl text-sm ${
                            attempt.passed ? "bg-green-50 dark:bg-green-900/10" : "bg-[var(--surface)]"
                          }`}>
                            {attempt.passed
                              ? <CheckCircle2 size={15} className="text-green-500 mt-0.5 shrink-0" />
                              : <Circle size={15} className="text-[var(--muted)] mt-0.5 shrink-0" />
                            }
                            <div className="min-w-0 flex-1">
                              <p className="text-[var(--text)] truncate">{attempt.feedback}</p>
                              <p className="text-xs text-[var(--muted)] mt-0.5">{new Date(attempt.createdAt).toLocaleTimeString()}</p>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {activeTab === "discussion" && (
                <div className="card p-5 space-y-4">
                  <div>
                    <p className="font-semibold text-sm text-[var(--text)]">Обсуждение шага</p>
                    <p className="text-xs text-[var(--muted)] mt-0.5">Задавайте вопросы по теории и решениям</p>
                  </div>
                      ...
                      <div key={msg.id} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-semibold text-[var(--text)]">{msg.author}</span>
                            <span className="text-xs text-[var(--muted)]">{new Date(msg.createdAt).toLocaleTimeString()}</span>
                          </div>
            ...
            <div className="xl:sticky xl:top-4 h-fit">
              <div className="card p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-semibold text-sm text-[var(--text)]">Содержание</p>
                </div>
                ...
                <div className="space-y-1.5 max-h-[calc(100vh-200px)] overflow-auto pr-1">
                  {syllabusModules.map((module) => (
                    <div key={module.id}>
                      ...
                      >
                        <span className="text-xs font-semibold text-[var(--text)]">{module.title}</span>
                          ...
                          {module.lessons.flatMap((lesson) =>
                            lesson.steps.map((step) => {
                              ...
                              return (
                                <button
                                  key={step.id}
                                  onClick={() => selectStep(step.id, prog?.answerText || "")}
                                ...
                                >
                                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${
                                    ...
                                    {!isDone && isActive && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
                                  </div>
  ...
  const clearFilters = () => { setSelectedCategory(null); setSelectedLevel(null); setActive("Все"); setSearch(""); };
  ...
  return (
    <MainLayout>
      ...
      >
        {/* Header */}
        <div>
          <h1 className="font-display font-bold text-3xl text-[var(--text)] mb-1">Каталог курсов</h1>
          <p className="text-[var(--muted)] text-sm">{courses.filter((c) => c.published !== false).length} курсов по разным направлениям</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {activeFiltersCount > 0 && (
              <button onClick={clearFilters} className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors border border-red-200 dark:border-red-800/30">
                <X size={11} /> Сбросить
              </button>
            )}
          </div>
          <button onClick={() => setCreateModalOpen(true)} className="btn-primary px-4 py-2 text-sm shrink-0">
            + Добавить курс
          </button>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <button
            ...
            className={`shrink-0 px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${
              !selectedCategory ? "bg-primary text-white border-primary" : "border-[var(--border)] text-[var(--muted)] hover:border-primary/40"
            }`}
          >
            Все
          </button>
            ...
            return (
              <button
                ...
                className={`shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${
                  isSelected ? "bg-primary text-white border-primary" : "border-[var(--border)] text-[var(--muted)] hover:border-primary/40"
                }`}
              >
                {meta.icon}
                {meta.label}
                <span className="text-[10px] opacity-60">{courses.filter((c) => c.category === cat).length}</span>
              </button>
            );
          })}
        </div>
        ...
        {/* Results count */}
        <p className="text-xs text-[var(--muted)]">{catalogFiltered.length} из {courses.length} курсов</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              return (
                ...
                >
                  ...
                  {course.coverUrl ? (
                    ...
                    <div className="h-32 bg-gradient-to-br from-primary to-burgundy flex items-center justify-center shrink-0">
                      <div className="text-white/40">
                        {cloneElement(catMeta.icon as React.ReactElement<{ size?: number }>, { size: 36 })}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    <div className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md mb-2 self-start ${catMeta.color}`}>
                    ...
                    <div className="mt-auto pt-3 space-y-2">
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-[var(--muted)]">
                        ...
                        <span className="flex items-center gap-0.5"><Clock3 size={10} />{course.duration}</span>
                      </div>
                      ...
                      {course.progress > 0 && (
                        <div>
                          ...
                          <p className="text-[10px] text-[var(--muted)] mt-0.5">{course.progress}% пройдено</p>
                        </div>
                      )}

                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-bold ${course.price === "Бесплатно" ? "text-green-600 dark:text-green-400" : "text-[var(--text)]"}`}>
                          {course.price}
                        </span>
                        {course.progress === 0 && (
                          <button
            ...
            {catalogFiltered.length === 0 && (
              <div className="col-span-full card p-10 flex flex-col items-center justify-center text-center">
                ...
                <p className="font-semibold text-[var(--text)] mb-1">Курсы не найдены</p>
                <p className="text-sm text-[var(--muted)] mb-4">
                  {search ? "Попробуйте изменить запрос или сбросить фильтры." : "Нет курсов в выбранной категории."}
                </p>
                {activeFiltersCount > 0 && (
                  <button onClick={clearFilters} className="btn-secondary px-4 py-2 text-sm">Сбросить фильтры</button>
                )}
              </div>
            )}
          </div>
        ...
        <Modal open={createModalOpen} onClose={() => setCreateModalOpen(false)} title="Создать курс">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <select value={newCourseType} onChange={(e) => setNewCourseType(e.target.value)} className="input-field px-4 py-3 text-sm">
                <option>Frontend</option>
                <option>Backend</option>
              </select>
              <select value={newCourseLevel} onChange={(e) => setNewCourseLevel(e.target.value)} className="input-field px-4 py-3 text-sm">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setCreateModalOpen(false)} className="btn-ghost px-4 py-2 text-sm">Отмена</button>
              <button onClick={createCourse} className="btn-primary px-4 py-2 text-sm">Создать</button>
            </div>
          </div>
        </Modal>
      </motion.div>
