...
export default function Course() {
  ...
  const loadCourseContent = async (courseId: number) => {
    ...
    try {
      ...
      const requestStepExists =
        requestedStepId &&
        data.steps.some((step) => step.id === requestedStepId);
      ...
      setAttemptHistory((prev) =>
        prev.filter((item) =>
          data.steps.some((step) => step.id === item.stepId),
        ),
      );
    } catch (err) {
      setContentError(
  ...
  const enrollCourse = async (courseId: number) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Не удалось записаться на курс";
  ...
  const submitStep = async () => {
    ...
    const currentStepIndex =
      courseContent?.steps.findIndex((step) => step.id === selectedStepId) ??
      -1;
  ...
  const activeStep =
    courseContent?.steps.find((step) => step.id === selectedStepId) ?? null;
  const activeProgress =
    courseContent?.progress.find((item) => item.stepId === selectedStepId) ??
    null;
  const activeStepIndex =
    courseContent?.steps.findIndex((step) => step.id === selectedStepId) ?? -1;
  const previousStep =
  ...
  const selectedAttempts = attemptHistory.filter(
    (item) => item.stepId === selectedStepId,
  );
  ...
  const remainingStepsCount = Math.max(
    (courseContent?.summary.total ?? 0) -
      (courseContent?.summary.completed ?? 0),
    0,
  );
  const averageStepXp = courseContent?.steps.length
    ? Math.round(
        courseContent.steps.reduce((sum, step) => sum + step.xp, 0) /
          courseContent.steps.length,
      )
    : 0;
  ...
  const selectStep = (stepId: number, answerText = "") => {
    ...
    if (window.innerWidth < 1280) {
      setTimeout(() => {
        document
          .getElementById("active-step-container")
  ...
  const scrollToSection = (section: "syllabus" | "discussion") => {
    const target =
      section === "syllabus"
        ? syllabusSectionRef.current
        : discussionSectionRef.current;
    if (!target) {
            ...
            <div className="rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white/90 dark:bg-slate-900/70 p-4 md:p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-slate-200/80 dark:border-slate-700/70 p-3 bg-slate-50/70 dark:bg-slate-800/50"
                  >
    ...
    return (
      <MainLayout>
        ...
        >
          <motion.div
            variants={fadeInUp}
            className="rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white/90 dark:bg-slate-900/70 p-4 md:p-5"
          >
            <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
              <div>
                ...
                <h2 className="text-xl md:text-2xl font-bold mt-1">
                  {courseContent?.course.title || "Прохождение курса"}
                </h2>
                <p className="text-sm text-slate-500 mt-1">
                  Формат: теория, мини-тесты и практические задания в одном
                  потоке.
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
              ...
              <div className="rounded-xl border border-slate-200/80 dark:border-slate-700/70 p-3 bg-slate-50/70 dark:bg-slate-800/50">
                ...
                <p className="text-lg font-semibold">
                  {courseContent?.summary.xp ?? 0} XP
                </p>
              </div>
              <div className="rounded-xl border border-slate-200/80 dark:border-slate-700/70 p-3 bg-slate-50/70 dark:bg-slate-800/50">
                <p className="text-xs text-slate-500">Прогресс</p>
                <p className="text-lg font-semibold">
                  {courseContent?.summary.percent ?? 0}%
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 rounded-xl border border-slate-200/80 dark:border-slate-700/70 p-3 bg-slate-50/70 dark:bg-slate-800/45">
              ...
              <p className="text-sm mt-1">
                Осталось шагов:{" "}
                <span className="font-semibold">{remainingStepsCount}</span>
                {estimatedRemainingXp > 0 ? (
                  <span className="text-slate-500">
                    {" "}
                    • примерно {estimatedRemainingXp} XP
                  </span>
                ) : null}
              </p>
            </div>
              ...
              enrollmentStatus.requestStatus === "pending" && (
                <div className="mt-3 rounded-xl border border-blue-200/80 dark:border-blue-700/40 p-3 bg-blue-50/70 dark:bg-blue-900/20">
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                    Преподаватель рассмотрит вашу заявку и примет решение.
                  </p>
                </div>
              )}
            ...
            {enrollmentStatus &&
              !enrollmentStatus.enrolled &&
              enrollmentStatus.requestStatus === "rejected" && (
                <div className="mt-3 rounded-xl border border-red-200/80 dark:border-red-700/40 p-3 bg-red-50/70 dark:bg-red-900/20">
                  ...
                  {enrollmentStatus.teacherComment && (
                    <p className="text-xs text-red-600 dark:text-red-400 mt-1">
                      Комментарий: {enrollmentStatus.teacherComment}
                    </p>
                  )}
          ...
          <motion.div
            variants={fadeInUp}
            className="sticky top-2 z-20 rounded-xl border border-slate-200/80 dark:border-slate-700/80 bg-white/95 dark:bg-slate-900/95 px-3 py-2"
          >
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <div className="text-sm text-slate-600 dark:text-slate-300">
                <span className="font-medium">Курсы</span> /{" "}
                <span>{courseContent?.course.title || "Курс"}</span> /{" "}
                <span className="text-slate-900 dark:text-slate-100">
                  {activeStep?.title || "Шаг"}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  onClick={submitStep}
                  disabled={stepLoading || !activeStep}
                >
                  {stepLoading ? "Проверка..." : "Проверить шаг"}
                </Button>
              </div>
            </div>
          </motion.div>
              ...
              <Card className="p-5 border border-slate-200/80 dark:border-slate-700/80">
                ...
                {!contentLoading && !contentError && courseContent && (
                  <div className="space-y-5">
                    ...
                    {!activeStep && (
                      <p className="text-sm text-slate-500">
                        Выберите шаг из оглавления справа.
                      </p>
                    )}
                    ...
                    {activeStep && (
                      <>
                        <div className="pb-3 border-b border-slate-200/80 dark:border-slate-700/80">
                          <p className="text-xs uppercase tracking-wide text-slate-500">
                            Шаг {activeStep.stepOrder} • {stepKindLabel} •{" "}
                            {activeStep.xp} XP
                          </p>
                          ...
                          <p className="text-sm mt-2 text-slate-700 dark:text-slate-200">
                            ...
                            <span className="font-semibold">
                              {activeStep.taskTypeLabel || stepKindLabel}
                            </span>
                          </p>
                          {activeProgress && (
                            <p className="text-xs text-slate-500 mt-2">
                              Попыток: {activeProgress.attempts} • Статус:{" "}
                          ...
                          <div className="space-y-3">
                            <div className="rounded-lg border border-slate-200/80 dark:border-slate-700/80 p-3 bg-slate-50/70 dark:bg-slate-800/40">
                              ...
                              <p className="text-sm text-slate-700 dark:text-slate-200 mt-1">
                                {activeStep.theoryText ||
                                  "Реализуйте решение согласно условию задания."}
                              </p>
                              <p className="text-xs text-slate-500 mt-2">
                                Проверка выполняется по{" "}
                                {activeStep.checkCount ||
                        ...
                        {diffPreview && (
                          <div className="rounded-lg border border-slate-200/80 dark:border-slate-700/80 p-3 bg-slate-50/70 dark:bg-slate-800/35">
                            <p className="text-sm font-semibold">
                              Изменения относительно последней попытки
                            </p>
                            <p className="text-xs text-slate-500 mt-1">
                              Изменено строк: {diffPreview.changed} • Добавлено:{" "}
                              {diffPreview.added} • Удалено:{" "}
                              {diffPreview.removed}
                            </p>
                          </div>
                        )}
                        ...
                        <div className="pt-3 border-t border-slate-200/80 dark:border-slate-700/80 flex items-center justify-between gap-2">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const progress = courseContent.progress.find(
                                (item) => item.stepId === previousStep.id,
                              );
                              selectStep(
                                previousStep.id,
                                progress?.answerText || "",
                        ...
                        <div className="pt-3 border-t border-slate-200/80 dark:border-slate-700/80 space-y-2">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <div className="flex gap-1">
                              {(["all", "passed", "failed"] as const).map(
                                (filter) => (
                                  <button
                                    key={filter}
                                    type="button"
                          ...
                          {visibleAttempts.length === 0 && (
                            <p className="text-sm text-slate-500">
                              Пока нет попыток по этому шагу.
                            </p>
                          )}
                          {visibleAttempts.slice(0, 6).map((attempt) => (
                            <div
                              key={attempt.id}
                              className="rounded-lg border border-slate-200/80 dark:border-slate-700/80 p-3"
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <span
                                  className={`text-xs px-2 py-1 rounded ${attempt.passed ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300"}`}
                                >
              ...
              >
                <Card className="p-5 border border-slate-200/80 dark:border-slate-700/80 space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Задавайте вопросы по материалу и решениям. Формулируйте
              ...
              <Card className="p-4 border border-slate-200/80 dark:border-slate-700/80 xl:sticky xl:top-24 h-fit">
                <div className="mb-3 flex items-center justify-between gap-2 min-w-0">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      ...
                      className="px-2 py-1 text-xs rounded-lg border border-slate-200/80 dark:border-slate-700/80 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    >
                      Развернуть
                    </button>
                    <button
                ...
                <div className="space-y-2 max-h-[70vh] overflow-auto pr-1">
                  {syllabusModules.map((module) => (
                    <div
                      key={module.id}
                      className="rounded-lg border border-slate-200/80 dark:border-slate-700/80 p-2"
                    >
                      <button
                        type="button"
                      ...
                      {openModules[module.id] && (
                        <div className="mt-2 space-y-2">
                          {module.lessons.map((lesson) => (
                            <div
                              key={lesson.id}
                              className="rounded-md bg-slate-50/80 dark:bg-slate-800/40 p-2"
                            >
                              <p className="text-xs uppercase tracking-wide text-slate-500">
                                {lesson.title}
                              </p>
                                ...
                                {lesson.steps.map((step) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`w-full text-left rounded-md p-2 border text-sm transition ${
  ...
  const CATEGORY_META: Record<
    string,
    { label: string; icon: React.ReactNode; gradient: string; color: string }
  > = {
    Programming: {
    ...
    Design: {
      ...
      color: "text-pink-600 bg-pink-50 dark:text-pink-400 dark:bg-pink-900/30",
    },
    "Data Science": {
      label: "Data Science",
  ...
  return (
    <MainLayout>
      ...
      >
        ...
        <motion.div
          variants={fadeInUp}
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-900 via-red-800 to-slate-900 dark:from-red-950 dark:via-slate-900 dark:to-black p-6 md:p-10"
        >
          ...
          <div className="relative">
            ...
            <p className="text-slate-300 mt-2 text-sm md:text-base max-w-xl">
              {publishedCoursesCount} курсов по программированию, дизайну,
              аналитике и другим направлениям
            </p>
                    ...
                    {course.coverUrl ? (
                      <div className="h-36 overflow-hidden relative">
                        ...
                        <div className="absolute top-2 left-2">
                          <span
                            className={`inline-flex items-center gap-1 text-[10px] px-2 py-1 rounded-lg font-medium backdrop-blur-sm bg-white/80 dark:bg-slate-900/80 ${catMeta.color}`}
                          >
                      ...
                      <div
                        className={`h-36 bg-gradient-to-br ${catMeta.gradient} flex flex-col items-center justify-center gap-2 relative`}
                      >
                        <div className="text-white/70">
                          {cloneElement(
                            catMeta.icon as React.ReactElement<{
                              size?: number;
                            }>,
                            { size: 36 },
                          )}
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        ...
                        {course.category && !course.coverUrl && (
                          <span
                            className={`text-[10px] px-1.5 py-0.5 rounded ${catMeta.color}`}
                          >
                            {catMeta.label}
                          </span>
                        )}
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <span
                          className={`text-sm font-bold ${course.price === "Бесплатно" ? "text-emerald-600 dark:text-emerald-400" : "text-slate-800 dark:text-slate-200"}`}
                        >
                          {course.price}
                        </span>
                        {course.progress === 0 && (
              ...
              <EmptyState
                ...
                description={
                  search
                    ...
                    : "Пока нет курсов в выбранной категории."
                }
              />
            </div>
          )}
