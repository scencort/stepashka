// страница восстановления пароля — шаг 1 из 2
// пользователь вводит свой email → бэк создаёт 6-значный код и отправляет на почту
// после успеха автоматически редиректим на /reset-password?email=...&devCode=...
//
// КАК РАБОТАЕТ:
//   handleSubmit() → validate() → POST /auth/forgot-password { email }
//     → бэк проверяет что пользователь с таким email существует
//       → создаёт код, сохраняет в БД, отправляет письмо (или в dev-режиме возвращает в ответе)
//       → возвращает { success, message, devCode?, devMode? }
//   после успеха: setSubmitted(true) → показываем "Письмо отправлено" зелёный блок
//   через 1.5 сек: navigate("/reset-password?email=...&devCode=...") — переходим на второй шаг
//   devCode передаётся в query чтобы ResetPassword.tsx мог подставить его автоматически
//
// ЗАЩИТА:
//   если email не существует — бэк всё равно возвращает success:true (не раскрываем существование)
//   validate() проверяет формат email регуляркой до отправки — экономит запросы
//
// ВИЗУАЛ:
//   AuthScreenShell — фоновый canvas с матричным дождём + кнопка смены темы
//   после setSubmitted(true) — форма заменяется зелёной иконкой CheckCircle2
