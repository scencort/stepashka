...
// публичная страница — доступна всем, в том числе незалогиненным
// маршрут: "/" (root) — первое что видит посетитель
//
// СЕКЦИИ (сверху вниз):
//   NAV       — sticky навбар с логотипом, ссылками, кнопкой темы и кнопками входа/регистрации
//               если юзер уже залогинен → кнопка "Войти в кабинет" вместо Login/Register
//   HERO      — главный слоган + кнопка регистрации + ParticleNetwork-фон
//               floating cards — декоративные карточки достижений вокруг заголовка (только xl:)
//   MARQUEE   — бегущая строка со статистикой платформы (курсы, студенты, рейтинг, участники)
//               stats берутся из GET /landing/stats, обновляются при монтировании
//               animate-marquee: бесконечный loop через CSS animation, данные дублированы 3× чтобы не было разрыва
//   COMMUNITY — секция с BinaryGlobe (вращающийся глобус из точек) + live activity feed
//               feed — массив псевдо-событий (mock), обновляется каждые 3.6 сек через setInterval
//               AnimatePresence mode="popLayout" — анимация при добавлении/удалении карточек в списке
//   FEATURES  — четыре карточки с возможностями платформы (Code/Brain/Shield/Trophy)
//   HOW IT WORKS — три шага "01 Запишитесь / 02 Решайте / 03 Получите сертификат"
//   TEAM      — карточки разработчиков с фото, ролями, ссылками на Telegram
//               на десктопе: TeamRevolver (круговое расположение карточек)
//               на мобиле: обычная сетка 2 колонки
//   CTA       — призыв к действию с кнопками регистрации
//   FOOTER    — ссылки на Privacy/Terms
//
// ПРОИЗВОДИТЕЛЬНОСТЬ:
//   globePalette = useMemo([theme]) — не пересоздаёт массив цветов на каждом ре-рендере
//   feed обновляется через setInterval 3600мс — не слишком часто
//   BinaryGlobe получает stable ссылку на palette → не пересоздаёт WebGL/canvas контекст лишний раз
//   img loading="lazy" на фото команды — не грузим всё сразу
