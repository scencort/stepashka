...
//   categories = useMemo → [...new Set(faqData.map(item => item.category))]
//     new Set() убирает дубликаты, [...] разворачивает обратно в массив
//     пересчитывается только когда изменился faqData
//   filtered = useMemo → сначала фильтр по категории, потом по query — оба работают одновременно
//     пересчитывается только когда изменился faqData, query или activeCategory
//
// КАК РАБОТАЕТ АККОРДЕОН:
//   openIds: Set<number> — Set позволяет держать несколько открытых вопросов одновременно
//   toggle(id): делаем копию Set (new Set(prev)), добавляем или удаляем id, возвращаем копию
//   React видит новый Set → перерисовывает список → {isOpen && <ответ>} показывает/скрывает
//   Set вместо массива: проверка .has(id) за O(1), у массива было бы .includes(id) за O(n)
//
// КАК РАБОТАЕТ AI-ПОИСК:
//   пользователь вводит вопрос в то же поле что и для фильтрации
//   нажимает "Спросить AI" или Enter → askAi() → POST /ai/faq { question: query }
//   ответ показывается отдельной карточкой над FAQ-списком
